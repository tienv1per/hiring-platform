  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Find the specific blog from the list
        ...
        if (blog) {
          ...
          // We need the full content, so we fetch it via the slug
          if (blog.slug) {
            try {
              // Try public endpoint first (only works for published blogs)
              const contentRes = await blogApi.get(`/api/blogs/${blog.slug}`);
              setContent(contentRes.data.content || "");
              // Set initial content for the editor
              if (contentRes.data.content) {
                setInitialContent(contentRes.data.content);
              }
            } catch {
              // Blog may be a draft, content won't be available via public API
              setContent("");
            }
          }
        }
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    {tags.map((tag) => (
                      <span key={tag} className="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 rounded-lg text-xs font-medium">
                        {tag}
